...
import com.elytradev.teckle.common.tile.networktiles.NetworkTileBreaker;
...
public class TileBreaker extends TileNetworkMember implements ITickable {

    public UUID bufferID;
    public AdvancedStackHandlerEntry bufferData;
    private NetworkTileBreaker networkTile;
    ...
    public void validate() {
        try {
            ...
            if (getNetworkTile() == null)
                this.setNetworkTile(new NetworkTileBreaker(this));
            ...
            TeckleLog.error("Failed to validate breaker. {}", e);
        }
    }
    ...
    public boolean tryPush() {
        ...
        try {
            ...
            if (!world.isRemote && (hasInsertionDestination || destinationIsAir)) {
                ...
                List<ItemStack> allData = getExtractionData(facing);

                for (ItemStack extractionData : allData) {
        ...
        cooldown = TeckleMod.CONFIG.breakerCooldown;
        return result;
    }
     ...
     */
    private List<ItemStack> getExtractionData(EnumFacing facing) {
        if (bufferData == null)
            return Collections.emptyList();
        NonNullList<ItemStack> extractionData = NonNullList.create();
        ...
        if (bufferSlot != -1) {
            extractionData.add(bufferData.getHandler().extractItem(bufferSlot, 1, false));
        } else {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide().isServer()) {
            ...
            if (networkID == null) {
                ...
                for (NodeContainer container : network.getNodeContainersAtPosition(pos)) {
                    if (container.getFacing() == null && container.getNetworkTile() instanceof NetworkTileBreaker) {
                        setNetworkTile((NetworkTileBreaker) container.getNetworkTile());
                        break;
                    }
                }
            }
    ...
    public boolean isPowered() {
        return world.getBlockState(pos).getValue(BlockBreaker.TRIGGERED);
    }

    protected NetworkTileBreaker getNetworkTile() {
        return networkTile;
    }

    protected void setNetworkTile(NetworkTileBreaker networkTile) {
