...
public class BlockBreaker extends BlockContainer {
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        ...
        if (tileentity instanceof TileBreaker) {
            if (powered) {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (tileAtPos != null && CapabilityWorldNetworkTile.isPositionNetworkTile(worldIn, pos, null)) {
            ...
            if (tileAtPos instanceof TileBreaker) {
                TileBreaker breaker = (TileBreaker) worldIn.getTileEntity(pos);

                // Vomit the buffer.
                breaker.bufferData.getHandler().stream().filter(stack -> !stack.isEmpty()).forEach(stack -> InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), stack));
                AdvancedStackHandlerPool.getPool(worldIn).remove(breaker.bufferData.getId());
            }
        }
